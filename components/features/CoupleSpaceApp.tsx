...
import { generateChatResponse, generateCoupleInvitationReaction } from '../../services/geminiService';
import { Contact, AppID } from '../../types';
...
const CoupleSpaceApp: React.FC<CoupleSpaceAppProps> = ({ onBack, isOpen, onOpenExternalApp }) => {
  ...
  // --- Dynamic Contact List State (Refactored to match Standalone Logic) ---
  const [availableCandidates, setAvailableCandidates] = useState<Contact[]>(() => {
      ...
      // 2. Define Defaults (System Chars)
      ...
      // 3. Merge & Deduplicate (Logic referenced from standalone.html)
      // We combine loaded contacts + defaults, then filter by unique ID
  ...
  // Ensure we update candidates if the modal is re-opened (in case user added contacts in ChatApp)
  useEffect(() => {
      if (isOpen && !partner) {
          const savedContacts = localStorage.getItem('ephone_contacts');
          const loadedContacts: Contact[] = savedContacts ? JSON.parse(savedContacts) : [];
  ...
  const [startDate, setStartDate] = useState<Date>(() => {
      ...
      return saved ? new Date(saved) : new Date(Date.now() - 1000 * 60 * 60 * 24 * 10); // Default 10 days ago
  });

  const [currentTime, setCurrentTime] = useState(new Date());
  const [intimacyLevel, setIntimacyLevel] = useState(1);
  const [intimacyExp, setIntimacyExp] = useState(45); // 0-100
      ...
      return (
        <div className="absolute inset-0 bg-white flex flex-col z-50 app-transition">
            ...
            <div className="flex-1 p-6 flex flex-col items-center justify-center space-y-8 bg-gradient-to-b from-pink-50 to-white">
                            ...
                            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                                <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
                                    {availableCandidates.length === 0 ? (
                                        <div className="text-gray-400 text-xs w-full text-center py-2">No contacts available. Create some in Chat first!</div>
                                    ) : (
                ...
                {inviteStep === 'sending' && (
                     <div className="text-center animate-pulse"><h3 className="text-lg font-bold text-gray-700">Sending...</h3></div>
                )}
  ...
  return (
    <div className="absolute inset-0 flex flex-col z-50 app-transition text-gray-800" style={containerStyle}>
       {/* Background Overlay */}
       {wallpaper && <div className="absolute inset-0 bg-white/30 backdrop-blur-sm z-0"></div>}

       {/* Header */}
       <div className="h-24 pt-12 px-4 flex items-center justify-between z-10">
          <button onClick={onBack} className="w-10 h-10 bg-white/50 backdrop-blur-md rounded-full flex items-center justify-center text-gray-700">
            <span className="text-xl">‹</span>
          </button>
          <h1 className="text-lg font-bold text-gray-800 tracking-wide drop-shadow-sm">Couple Space</h1>
       ...
       {/* Scrollable Content */}
       <div className="flex-1 overflow-y-auto no-scrollbar pb-8 px-4 space-y-5 z-10">
          ...
          <div className="grid grid-cols-4 gap-2">
              {[
                { icon: '📅', label: 'Calendar', app: AppID.Calendar },
                { icon: '📱', label: 'Our Phone', app: AppID.Phone },
                { icon: '📔', label: 'Diary', app: undefined },
                { icon: '🎁', label: 'Gift', app: undefined }
              ].map((item, i) => (
                  <div 
                    key={i} 
                    onClick={() => {
                        if (item.app && onOpenExternalApp) onOpenExternalApp(item.app);
                    }}
